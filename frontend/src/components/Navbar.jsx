...
export default function Navbar() {
    ...
    useEffect(() => {
        const loadProfilePic = async () => {
            ...
            try {
                ...
                setProfilePicUrl(data?.profilePic ? `http://localhost:5000/uploads/${data.profilePic}` : "");
            } catch {
                setProfilePicUrl("");
            }
        };
    ...
    // Reusable Theme Toggle Button for Dropdowns
    const ThemeToggleButton = () => (
        <button
            className="nav-link profile-logout-btn dropdown-item-flex"
            onClick={(e) => {
                e.stopPropagation(); // Prevents the dropdown from closing when toggled
                toggleTheme();
            }}
        >
        ...
        <>
                        ...
                        <div className="nav-right">
                            {!user ? (
                                <div className="profile-not-logged">
                                    <button className="guest-text-btn" onClick={() => setGuestMenuOpen((v) => !v)}>Log in</button>
                                    <button className="profile-trigger" onClick={() => setGuestMenuOpen((v) => !v)}><span className="profile-fallback">?</span></button>
                                    {guestMenuOpen && (
                                ...
                                <div className="profile-menu-wrap">
                                    <button className="guest-text-btn" onClick={() => setUserMenuOpen((v) => !v)}>{user.name}</button>
                                    <button className="profile-trigger" onClick={() => setUserMenuOpen((v) => !v)}>
                                    ...
                                    {userMenuOpen && (
                                        <div className="profile-dropdown">
                                            ...
                                            <ThemeToggleButton />
                                            <button className="nav-link profile-logout-btn" onClick={logout}>Logout</button>
                                        </div>
                                    )}
                                </div>
                            )}
            ...
            <nav className="mobile-fb-tabs" aria-label="Mobile navigation">
                ...
                <NavLink to="/blog" className={({ isActive }) => `tab-item ${isActive ? "active" : ""}`}>
                    <svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h9.5" />
                        <path d="M17 2.5a2.121 2.121 0 0 1 3 3L12 13l-4 1 1-4 9.5-9.5z" />
                    </svg>
                </NavLink>
                ...
                {/* Fixed Mobile Profile Dropdown (Handles both Guest and User now) */}
                {user ? (
                    <div className="mobile-profile-wrap" style={{ display: "flex", width: "100%", height: "100%" }}>
                        <button
                            className={`tab-item profile-tab ${userMenuOpen ? "active" : ""}`}
                    ...
                    <div className="mobile-profile-wrap" style={{ display: "flex", width: "100%", height: "100%" }}>
                        <button
                            className={`tab-item profile-tab ${guestMenuOpen ? "active" : ""}`}
